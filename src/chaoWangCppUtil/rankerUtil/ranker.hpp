...
template <class T>
inline T sampleQuantile(const T* d, const uint size, const double q)
{
...
template <class T>
inline T sampleQuantile(const vector<T>& v, const double q)
  { return sampleQuantile(&v[0], v.size(), q); }

template <class T>
inline std::vector<T> sampleQuantile(const vector<T>& v, const std::vector<double> qs)
{ std::vector<T> ret;
	auto q = qs.cbegin();
	while(q!=qs.cend())
	{
		ret.push_back( sampleQuantile(&v[0], v.size(), *q++) );
	}
	return ret; 
}
